...
def initialize_grid_and_boundaries(Nx, Ny, h, xmin, ymin, Lx, Ly):
    ...
    # V(x, ymax) = 2 * x * log(2*x) where ymax = ymin + Ly
    for i in range(Nx + 1):
        x = xmin + i * h
        if x <= 0 or (4 * x) <= 0: # Should not happen with xmin=1
            phi[i, Ny] = 0.0
        else:
            phi[i, Ny] = 2.0 * x * np.log(4.0 * x)

    # V(xmax, y) = 2 * y * log(2*y) where xmax = xmin + Lx
    for j in range(Ny + 1):
        y = ymin + j * h
        if y <= 0 or (2 * y) <= 0: # Should not happen with ymin=1
            phi[Nx, j] = 0.0
        else:
            phi[Nx, j] = 2.0 * y * np.log(2.0 * y)
            
    # Calculate source term rho(x,y) = x/y + y/x for ALL points initially
...
def save_results(Nx, Ny, h, xmin, ymin, phi, filename="data_poisson-04-python.txt"):
    """
...
if __name__ == "__main__":
    ...
    xmin = 1.0
    ymin = 1.0
    Lx = 1.0  # Length of domain in X (xmax = xmin + Lx = 2.0)
    ...
    print('-------------------------------------------------------')
    print(' Resolviendo la ecuacion de Poisson bidimensional (Python/NumPy)')
    print(f' Dominio: [{xmin:.2f}, {xmin+Lx:.2f}] x [{ymin:.2f}, {ymin+Ly:.2f}]')
    print(f' Puntos en la grilla: {Nx+1} X {Ny+1}')
    print(f' Tamanio de paso h = {h:.5f}')
    print(f' Criterio de convergencia eps = {eps:.1e}')
    print(f' Termino fuente rho(x,y) = x/y + y/x')
    print(f' Condiciones de frontera Dirichlet:')
    print(f'  V(x,{ymin:.2f}) = x * log(x)')
    print(f'  V({xmin:.2f},y) = y * log(y)')
    print(f'  V(x,{ymin+Ly:.2f}) = 2 * x * log(2 * x)')
    print(f'  V({xmin+Lx:.2f},y) = 2 * y * log(2 * y)')
    print('-------------------------------------------------------')
