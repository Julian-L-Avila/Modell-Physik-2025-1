PROGRAM Poisson2D_ShiftedDomain
  ...
  integer,parameter::Ny = 100        ! Número de intervalos en Y (para Lx=1, Ly=1 => Nx=Ny para hx=hy)
  ...
  ymin = 1.0d0  ! Coordenada minima en Y
  Lx = 1.0d0    ! Longitud del dominio en X (xmax = xmin + Lx = 2.0)
  ...
  write(6,"(a)")' Termino fuente rho(x,y) = x/y + y/x'
  write(6,*)' Condiciones de frontera Dirichlet:'
  write(6,"(a,f6.2,a)")'  V(x,',ymin,') = x * log(x)'
  write(6,"(a,f6.2,a)")'  V(',xmin,',y) = y * log(y)'
  write(6,"(a,f6.2,a)")'  V(x,',ymin+Ly,') = 2 * x * log(4 * x)'
  write(6,"(a,f6.2,a)")'  V(',xmin+Lx,',y) = 2 * y * log(2 * y)'
  write(6,*)'-------------------------------------------------------'
...
SUBROUTINE initialize_grid_and_boundaries(Nx,Ny,h,xmin,ymin,Lx,Ly,is_boundary,phi,rho)
  ...
  ! Frontera inferior (y=ymin): V(x, ymin) = x * log(x)
  do i=0,Nx
     x = xmin + i * h
     if (x <= 0.0d0) then
  ...
  ! Frontera izquierda (x=xmin): V(xmin, y) = y * log(y)
  do j=0,Ny
     y = ymin + j * h
     if (y <= 0.0d0) then
  ...
  ! Frontera superior (y=ymax): V(x, ymax) = 2 * x * log(4*x)
  do i=0,Nx
     x = xmin + i * h
  ...
  ! Frontera derecha (x=xmax): V(xmax, y) = 2 * y * log(2*y)
  do j=0,Ny
     y = ymin + j * h
     if (y <= 0.0d0 .or. (2.0d0 * y) <= 0.0d0) then
        phi(Nx,j) = 0.0d0 ! Or handle error
     else
        phi(Nx,j) = 2.0d0 * y * log(2.0d0 * y)
     endif
  enddo
